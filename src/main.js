...
function CompoundElement({ children, yOff, xOff, extraData, zoom, maxZoom, zoomAffectOffset }) {
	...
	return withContext((context) => {
		...
		// the drawX and drawY should always be affected by currentZoom
		// as they are generated by our min position of elements, which
		// will probably be affectedby the zoom (as in, the user will be
		// drawing them with positions corresponding to max zoom)
		return (
			<Image
				src={renderRef.current.canvas}
				width={width*currentZoom}
				height={height*currentZoom}
				x={(useXOff+drawX*currentZoom)}
				y={(useYOff+drawY*currentZoom)}
			/>
		);
	});
}
